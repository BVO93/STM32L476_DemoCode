...
// scheduler decides on what box it transmits

void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *hcan){

	memset(msg, 0, sizeof(msg));
	sprintf(msg, "Transmit Of CAN is done M0 ! \n");
	lenOfData = strlen(msg);
	HAL_UART_Transmit(&huart2, (uint8_t*) msg, lenOfData, HAL_MAX_DELAY);
...
void HAL_CAN_TxMailbox1CompleteCallback(CAN_HandleTypeDef *hcan){
	...
	memset(msg, 0, sizeof(msg));
	sprintf(msg, "Transmit Of CAN is done M1 ! \n");
	lenOfData = strlen(msg);
	HAL_UART_Transmit(&huart2, (uint8_t*) msg, lenOfData, HAL_MAX_DELAY);
...
void HAL_CAN_TxMailbox2CompleteCallback(CAN_HandleTypeDef *hcan){
	...
	memset(msg, 0, sizeof(msg));
	sprintf(msg, "Transmit Of CAN is done M2! \n");
	lenOfData = strlen(msg);
	HAL_UART_Transmit(&huart2, (uint8_t*) msg, lenOfData, HAL_MAX_DELAY);
