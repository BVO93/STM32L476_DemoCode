...
/* ************************
 *
 * PROGRAM NOT COmplelty running
 * Stil have to find out why
 * I assume because on the L476 board the clock can not be so high
 *
 */
...
void TIMER6_init(void){
	...
	if (HAL_TIM_IC_Init(&htimer6) != HAL_OK) {
		Error_handler();
	}
